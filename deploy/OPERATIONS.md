...
through SSM. `api.aurumpos.net` points to the Elastic IP. Caddy manages TLS.
...
the migration from the same image, starts API/worker/Caddy, and checks readiness.
...
The existing private BMR deployment uses a mutable `latest` image and has no
worker service. Keep it unchanged only as the isolated rollback environment.
The SaaS cutover deploys `compose.cloud.yml` with a digest-pinned API and worker;
it must not run the destructive non-item reset against the live BMR database.
