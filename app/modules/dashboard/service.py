...
async def get_dashboard_summary(db: AsyncSession) -> dict:
    # Sum inventory quantity and total net weight (accounting for quantity)
    stmt = select(
        # Sum quantities instead of counting item records
        func.coalesce(func.sum(Item.quantity), 0),
        # Sum net weight multiplied by quantity for each item
        func.coalesce(func.sum(Item.net_weight * Item.quantity), 0),
    ).where(Item.status == "in_stock")
    result = await db.execute(stmt)
    inventory_count, total_net_weight = result.one()

    inventory_count = int(inventory_count or 0)
    total_net_weight = float(total_net_weight or 0)

    # Get the current Silver rate
    Silver_rate = await get_latest_metal_rate(
        db,
        metal="Silver",
        purity=100.0,
    )

    Silver_rate_per_gram = float(Silver_rate.rate_per_gram) if Silver_rate else 0.0
    ...
    total_sales_amount = float((await db.execute(sales_stmt)).scalar_one() or 0)

    # Fetch all latest metal rates to construct a rate lookup map
    ...
    # Get all in_stock items to calculate total sale value
    ...
    for item in in_stock_items:
        # Determine the rate per gram for this item
        ...
        total_sale_value += suggested_price * quantity
    ...
    return {
        ...
        "Silver_rate_per_10g": Silver_rate_per_10g,
...
async def get_dashboard_analytics(
    ...
    async def get_metrics_at_timestamp(T: datetime):
        ...
        stmt = (
            select(Item, sold_after_subquery.label("sold_after"))
            .where(Item.created_at <= T)
        )
        if metal.lower() != "all":
    ...
    total_sale_value_change = calc_change_pct(curr_metrics["total_sale_value"], prev_metrics["total_sale_value"])
    inventory_items_change = calc_change_pct(curr_metrics["inventory_items"], prev_metrics["inventory_items"])
    silver_rate_change = calc_change_pct(curr_metrics["silver_rate_10g"], prev_metrics["silver_rate_10g"])
    total_stock_value_change = calc_change_pct(curr_metrics["total_stock_value"], prev_metrics["total_stock_value"])
    ...
    for cat, val in cat_totals.items():
        ...
        sales_by_category.append({
            "category": cat,
            "sales_value": round(val, 2),
            "share": round(share, 1)
        })
        
    sales_by_category.sort(key=lambda x: x["sales_value"], reverse=True)
