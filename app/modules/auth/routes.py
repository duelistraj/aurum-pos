...
async def update_device_status(
    device_id: UUID, update_data: DeviceUpdate, db: AsyncSession = Depends(get_db)
):
...
async def verify_manager_password(request_data: VerifyManagerPasswordRequest):
    ...
    is_valid = secrets.compare_digest(
        request_data.password.strip(),
        settings.manager_password.strip(),
    )
